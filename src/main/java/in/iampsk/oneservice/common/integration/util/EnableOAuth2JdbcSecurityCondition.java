...
public class EnableOAuth2JdbcSecurityCondition implements Condition {
	...
	public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
		...
		if (oneServiceBootClass.isAnnotationPresent(EnableOAuth2JdbcSecurity.class)) {
			try {
				if (!oneServiceBootClass.isAnnotationPresent(EnableJdbc.class)) {
					message = "EnableOAuth2JdbcSecurity Requires JDBC enable. Please add @EnableJdbc";
					LOGGER.error(METHOD, message);
					throw new IllegalArgumentException(message);
				}else {
					if (oneServiceBootClass.isAnnotationPresent(IgnoreSecurity.class) || oneServiceBootClass.isAnnotationPresent(EnablePropSecurity.class) || oneServiceBootClass.isAnnotationPresent(EnableJdbcSecurity.class) || oneServiceBootClass.isAnnotationPresent(EnableOAuth2JdbcServer.class)) {
